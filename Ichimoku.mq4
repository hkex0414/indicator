                ...
                const int &spread[])
{
   ...
   if (IsCloud)
   {
      for (int i = limit - 1; i >= 0; i--)
      {
         ...
         //---- Tenkan Sen
         double Tenkan_Buffer = (High[iHighest(NULL, 0, MODE_HIGH, Tenkan, shift)] + Low[iLowest(NULL, 0, MODE_LOW, Tenkan, shift)]) / 2;
         //---- Kijun Sen
         double Kijun_Buffer = (High[iHighest(NULL, 0, MODE_HIGH, Kijun, shift)] + Low[iLowest(NULL, 0, MODE_LOW, Kijun, shift)]) / 2;
         //---- Senkou Span A
         SpanA_Buffer[i] = (Tenkan_Buffer + Kijun_Buffer) / 2;
         //---- Senkou Span B
         SpanB_Buffer[i] = (High[iHighest(NULL, 0, MODE_HIGH, Senkou, shift)] + Low[iLowest(NULL, 0, MODE_LOW, Senkou, shift)]) / 2;
         //----kumo
...
void calcSenkouSpan(int period, bool isNewCandle)
{
   ...
   //---- Senkou Span B
   double SpanB_Buffer = (High[iHighest(NULL, 0, MODE_HIGH, Senkou, shift)] + Low[iLowest(NULL, 0, MODE_LOW, Senkou, shift)]) / 2;
