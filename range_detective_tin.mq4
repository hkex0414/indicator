...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
  ...
  {
    Ema62[i] = iMA(NULL, 0, 60, 0, MODE_SMA, PRICE_CLOSE, i);
    Sma144[i] = iMA(NULL, 0, 100, 0, MODE_SMA, PRICE_CLOSE, i);
    Sma200[i] = iMA(NULL, 0, 200, 0, MODE_SMA, PRICE_CLOSE, i);
    Sma800[i] = iMA(NULL, 0, 800, 0, MODE_SMA, PRICE_CLOSE, i);
  }

  double nowEma62 = iMA(NULL, 0, 60, 0, MODE_SMA, PRICE_CLOSE, 0);
  double nowSma144 = iMA(NULL, 0, 100, 0, MODE_SMA, PRICE_CLOSE, 0);
  ...
  if (IsPushNotify)
  {
    ...
    {
      SendNotification(Symbol() + "RangeRush");
      Alert(Symbol() + "RangeRush");
    }
    if (b4IsRangeStr && !isRange)
    {
      SendNotification(Symbol() + "RangeBreak");
      Alert(Symbol() + "RangeBreak");
    }
  }
